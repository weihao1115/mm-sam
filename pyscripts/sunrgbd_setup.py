...
def download_file(url, output_filepath, display_progressbar=False):
    with DownloadProgressBar(
            unit='B', unit_scale=True, miniters=1, desc=url.split('/')[-1], disable=not display_progressbar
    ) as t:
        urllib.request.urlretrieve(url, filename=output_filepath, reporthook=t.update_to)


if __name__ == '__main__':
    output_path = os.path.expanduser(f"{DATA_ROOT}/sunrgbd")
    ...
    if not os.path.exists(toolbox_dir):
        ...
        download_file(DATASET_TOOLBOX_URL, zip_file_path, display_progressbar=True)
        with ZipFile(zip_file_path, 'r') as zip_ref:
            zip_ref.extractall(os.path.dirname(zip_file_path))
        os.remove(zip_file_path)

    data_dir = os.path.join(output_path, 'SUNRGBD')
    ...
    for i, meta in tqdm(enumerate(SUNRGBDMeta)):
        ...
        if not os.path.exists(label_path_full):
            ...
            label = np.array(SUNRGBD2Dseg[seglabel[i][0]][:].transpose(1, 0)).astype(np.uint8)
